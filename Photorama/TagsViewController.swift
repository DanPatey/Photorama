...
class TagsViewController: UITableViewController {
    ...
    @IBAction func addNewTag(sender: AnyObject) {
        ...
        alertController.addTextFieldWithConfigurationHandler({ (textField) -> Void in textField.placeholder = "tag name"
            textField.autocapitalizationType = .Words})
        
        let okAction = UIAlertAction(title: "OK", style: .Default, handler: { (action) -> Void in
        
            if let tagName = alertController.textFields?.first! {
                ...
                do { try self.store.coreDataStack.saveChanges()
                } catch let error {
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        do {
            ...
            print("Core Data has save failed: \(error)")
        }
    }
